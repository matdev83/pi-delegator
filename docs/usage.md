...
| `recoveryInactivitySeconds` | Idle-time recovery probe for inline-backend runs only (other backends ignore it). Defaults to 300 seconds (5 minutes) of silence before the session is asked whether it still has tasks; `0` disables it. If the session replies with the finish marker `"I HAVE FULLY FINISHED ALL TASKS FROM THIS SESSION"` (matched fuzzily), the run completes successfully; a session that resumes operations keeps the tool call open; no reaction within 3 minutes flags the session dead with the same `timeout` failure kind as the 15-minute guard. |
